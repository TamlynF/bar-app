...
**Event carries its calendar** (new columns on `stock_market_events`, see §1). A stock market event is a reusable setup that can be opened again and again, so nothing here restricts *when* it may run. `weekdays int[]` (0–6) says which day(s) of the week the event is *for*, so the right weekday's sales profile is used; `bank_holiday_profile int null` says which weekday's profile to use when a run date is the eve of a UK bank holiday (e.g. a bank-holiday Sunday trades like a Saturday); the history sampled is always the rolling last 12 weeks. Open/close times are for display and the ticks-per-night hint only — they do not bound the sales counted. If a session is opened on a weekday not in `weekdays`, the profile for the actual weekday is used and the staff panel says so.
...
2. Sales come from `square_sale_lines`, one row per Square order line that the nightly `/api/square/sync` cron writes alongside `square_sales` (with a manual "Sync sales from Square" button on the event page). Each line carries a `trading_night`: the `Europe/London` date it closed on, rolled back a day when the local time is before 06:00 (`NIGHT_ROLLOVER_HOUR`). So a Saturday night is every sale from Saturday 06:00 to Sunday 06:00, and Sunday-morning sales belong to the *Saturday* profile, never to Sunday's. Working the figures out is one local query per weekday — no Square call, so it also runs at session open.
   The same trading-night rule is used everywhere a "night" is meant: the night of a session is the night it *opened* on (a session opened Saturday 21:00 or Sunday 00:30 is a Saturday), the market-night exclusion and the bank-holiday-eve check. The live tick's own Square orders poll is watermark-based, not date-based.
