...
- **Admin action buttons have a fixed colour identity.** Any admin **Edit** button's `className` must contain `bg-[#B45309] hover:bg-[#B45309]/85 text-white font-black uppercase tracking-widest`; any **Save, Add, Create, New, or Upload** button's must contain `bg-[#1B4332] hover:bg-[#1B4332]/85 text-white font-black uppercase tracking-widest` (compose sizing/radius around them). `#1B4332` green = any record-creating/saving action; `#B45309` amber = Edit. Don't substitute espresso (`#5C4033`) for these. See STYLE_GUIDE "Action buttons".
...
| `private_hire_requests` | Private hire enquiries — status: `pending`, `confirmed`, `cancelled` (same set as `band_booking_requests`). `confirmed` (with a date) places/updates an active `events` row; `cancelled` deactivates the linked event |
