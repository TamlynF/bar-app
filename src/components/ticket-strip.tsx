        ...
        <ol
          className={cn(
            ...
            nights.length >= 3
              ? "grid-cols-3"
              : nights.length === 2
                ? "grid-cols-2 lg:max-w-4xl"
                : "grid-cols-1 lg:max-w-md",
          )}
        >
          {nights.map((night, g) => {
              ...
              >
                <header className="flex items-center justify-between gap-3 px-1 pt-1">
                  <div className="flex items-baseline gap-2">
                    ...
                    <span className="font-black text-3xl leading-none tracking-tighter text-ink tabular-nums">
                      {format(date, "d")}
                    </span>
                      ...
                      >
                        ...
                        >
                          {e.imageUrl ? (
                            ...
                            <span
                              className="absolute inset-0 bg-linear-to-br from-(--ev-c)/60 to-canvas"
                              aria-hidden="true"
                            />
                          )}
                        </Link>
                        ...
                        >
                          ...
                          <span className="mt-0.5 block truncate font-black text-sm tracking-tight text-ink uppercase">
                            {e.title}
                          </span>
                          <span className="block truncate text-[11px] text-ink-2">
                            {[e.subType, entryText(e)]
                              .filter(Boolean)
                              .join(" · ")}
                          </span>
                        </Link>
...
function Ticket({
  ...
  const time = e.startTimeLabel
    ? `${e.startTimeLabel}${e.endTimeLabel ? ` – ${e.endTimeLabel}` : ""}`
    : null;
  return (
    <VinylPeek color={e.color} className="[--peek-base:5%]">
      <article
        style={{ "--ev-c": e.color } as React.CSSProperties}
        className="group relative flex flex-col overflow-hidden rounded-[20px] border border-hairline bg-canvas-2 hover:border-gold/40"
      >
        <Link
          href={`/whats-on/${e.id}`}
          className="relative block h-44 overflow-hidden bg-canvas lg:h-50"
        >
          {e.imageUrl ? (
            <Image
              src={e.imageUrl}
              ...
              className="object-cover object-[center_30%] transition-transform duration-300 ease-out group-hover:scale-[1.03]"
            />
          ) : (
            <span
              className="absolute inset-0 bg-linear-to-br from-(--ev-c)/50 via-canvas-2 to-canvas"
              aria-hidden="true"
            />
          )}
          <span
            className="absolute inset-0 bg-linear-to-t from-canvas/85 to-canvas/10"
            aria-hidden="true"
          />
          {e.subType && (
          ...
          <span
            className={cn(
              "absolute bottom-3.5 left-4 font-black text-sm tracking-wide tabular-nums",
        ...
        <div className="flex flex-1 flex-col p-4">
          ...
          <p className="mt-2 truncate text-xs text-ink-2">
            {[e.tagline, entryText(e)].filter(Boolean).join(" · ")}
          </p>
