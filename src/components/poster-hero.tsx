    ...
    >
      ...
      <div className={cn("absolute top-3 left-3 flex flex-col items-start gap-2.5 sm:top-22 sm:left-6 lg:left-10", isTonight && "max-md:hidden")}>
        ...
        >
          ...
          <span className={cn("text-eyebrow font-semibold", isTonight ? "text-ink" : "text-gold")}>
      ...
      <div className="absolute top-3 right-3 flex rotate-3 flex-col items-center rounded-[10px] border border-gold/40 bg-canvas/80 px-2.5 py-1.5 leading-none text-ink shadow-lg shadow-black/50 backdrop-blur-md md:hidden">
        <span className="text-pill font-semibold tracking-wide text-gold uppercase">{isTonight ? "Tonight" : format(dateObj, "EEE")}</span>
        <span className="mt-1 font-black text-[22px] tracking-tighter tabular-nums">{format(dateObj, "d")}</span>
        <span className="mt-0.5 text-pill font-semibold tracking-wide uppercase">{format(dateObj, "MMM")}</span>
      </div>
      ...
      <div className="absolute right-3 bottom-4 left-3 flex flex-col gap-2.5 sm:right-6 sm:left-6 md:right-auto md:bottom-16 md:w-[60%] md:gap-5 lg:left-10">
        <div className="flex items-center gap-2 sm:gap-2.5">
          {event.subType && (
            <span className="-rotate-2 rounded-md bg-(--ev-c) px-2 py-1 text-pill font-bold tracking-wide text-canvas uppercase shadow-lg shadow-black/40 sm:px-2.5">
              {event.subType}
            </span>
          )}
          <span
            className={cn(
              "animate-[ad-fade-in_1s_ease-in_both] rounded-full px-2.5 py-1 text-pill font-bold tracking-wide uppercase shadow-lg shadow-black/40 sm:px-3",
        ...
        <div onPointerDown={neonFlicker} className="ad-pulse-cta mt-1 flex items-stretch gap-2.5 [&_a]:h-12 [&_a]:w-auto [&_a]:flex-1 [&_a]:px-5 [&_a]:text-btn [&_span]:h-12 [&_span]:w-auto [&_span]:flex-1 [&_span]:px-4 sm:gap-3 md:[&_a]:h-14 md:[&_a]:flex-none md:[&_a]:px-8 md:[&_span]:h-14 md:[&_span]:flex-none md:[&_span]:px-7">
      ...
      {acts > 1 && (
        <div className="absolute right-6 bottom-16 hidden w-85 overflow-hidden rounded-2xl border border-ink/18 bg-canvas/80 shadow-[0_24px_50px_-20px_rgba(0,0,0,0.8)] backdrop-blur-xl md:block lg:right-10">
          <div className="flex items-center justify-between border-b border-ink/10 px-4.5 pt-3.5 pb-2.5">
            <span className="text-eyebrow font-semibold text-gold">
              {isTonight ? "Tonight's bill" : `${format(dateObj, "EEEE")}'s bill`}
            </span>
            <span className="text-meta text-ink-2">{acts} acts · one entry</span>
          </div>
              ...
              return (
                <li key={e.id}>
                  ...
                  >
                    ...
                    {on ? (
                      <span className="rounded-full bg-gold px-2 py-1 text-pill font-bold tracking-wide text-on-gold uppercase">Showing</span>
                    ) : (
