...
export function MarketPill() {
  ...
  const label = deal ? `Drinks market open: ${deal.name} now ${formatGbp(price)}` : "Drinks market open";

  const icon = crash ? <Zap className="h-5 w-5" aria-hidden="true" /> : <TrendingDown className="h-5 w-5" aria-hidden="true" />;
  const iconClass = cn(
    "relative flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-full text-on-gold",
    crash ? "bg-linear-to-br from-neon to-[#E6392E] text-ink" : "bg-linear-to-br from-gold to-neon"
  );

  if (!expanded) {
    return (
      <div ref={rootRef} className="pointer-events-none fixed right-0 bottom-0 z-40 pr-5 pb-[calc(env(safe-area-inset-bottom)+1.25rem)] sm:hidden">
        <button
          type="button"
        ...
        >
          <span className="ad-shimmer pointer-events-none absolute inset-0 opacity-80" aria-hidden="true" />
          {icon}
          <span className="ad-live-dot absolute top-0.5 right-0.5 h-2.5 w-2.5 rounded-full bg-[#E6392E] ring-2 ring-canvas" aria-hidden="true" />
        </button>
      </div>
    );
  }
  ...
  return (
    <div ref={rootRef} className="pointer-events-none fixed inset-x-0 bottom-0 z-40 flex justify-end px-5 pb-[calc(env(safe-area-inset-bottom)+1.25rem)] sm:hidden">
      <div
        className={cn(
      ...
      >
        <span className="ad-shimmer pointer-events-none absolute inset-0 opacity-60" aria-hidden="true" />

        {/* Body: the deal - tap to go to the market */}
        <Link
          href="/market"
          aria-label={`${label}. Open the market`}
          className="group relative flex min-w-0 items-center gap-2 py-1.5 pr-2 pl-3.5 active:scale-[0.98]"
        >
            ...
            <span className="mt-1 flex items-baseline gap-1.5 font-black text-[13px] tracking-tight uppercase">
              {deal ? (
                <>
                  ...
                  <span key={price ?? 0} className="ad-odometer inline-block text-gold tabular-nums">
                    {formatGbp(price)}
                  </span>
                  {deal.changePct < 0 && (
                    <span className="text-[10px] text-neon tabular-nums">↓{Math.abs(Math.round(deal.changePct))}%</span>
                  )}
                </>
              ) : (
                "Buy the dips"
          ...
          <ArrowRight className="relative h-4 w-4 shrink-0 text-gold transition-transform group-active:translate-x-0.5" aria-hidden="true" />
        </Link>
