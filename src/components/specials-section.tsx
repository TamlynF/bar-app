...
export function SpecialsSection({ specials }: { specials: SpecialRow[] }) {
  ...
  return (
    <section id="specials" className="scroll-mt-24">
      ...
      <ul className="no-scrollbar -mx-4 flex snap-x snap-mandatory scroll-px-4 items-stretch gap-4 overflow-x-auto px-4 pb-3 sm:mx-0 sm:grid sm:grid-cols-2 sm:overflow-visible sm:px-0 sm:pb-0 xl:grid-cols-4">
          ...
          <SpecialStub
            ...
            onToggle={() =>
              setFlippedId((currentId) =>
                currentId === special.id ? null : special.id
              )
            }
          />
        ))}
...
function SpecialStub({
  ...
  const badges = isNew
    ? special.badges.filter((badge) => badge.trim().toLowerCase() !== "new")
    : special.badges;

  const backId = `special-${special.id}-details`;

  return (
    <li className="h-full w-[min(86vw,340px)] shrink-0 snap-start perspective-[1600px] sm:w-auto sm:min-w-0 sm:shrink sm:snap-none">
      <div
        className={cn(
      ...
      >
        ...
        <button
          ...
          className={cn(
            "group flex min-h-72 w-full flex-col overflow-hidden rounded-3xl border border-[#7A1F1F]/45 bg-[#241512] text-left text-[#ffeede] shadow-lg shadow-black/30 backface-hidden transition-all duration-300 hover:-translate-y-1 hover:border-[#7A1F1F]/80 hover:shadow-2xl hover:shadow-black/50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#FDCC4B]",
        ...
        >
          ...
          <div className="relative z-10 flex flex-1 flex-col p-5">
            ...
            <h3 className="line-clamp-2 min-h-10 font-black text-xl leading-none tracking-tight text-white uppercase">
              {special.title}
            </h3>
            ...
            <div className="mt-auto flex min-w-0 items-end justify-between gap-3">
              ...
              <span className="ml-auto inline-flex min-h-11 shrink-0 items-center gap-2 pl-2 font-black text-[10px] tracking-widest text-[#FDCC4B] uppercase">
                Details
                <FlipHorizontal2
                  className="h-5 w-5 transition-transform duration-300 group-hover:rotate-y-180"
                  aria-hidden="true"
                />
              </span>
            </div>
        ...
        <div
          ...
          aria-hidden={!flipped}
          className={cn(
            "flex min-h-72 w-full flex-col overflow-hidden rounded-3xl border border-[#7A1F1F]/45 bg-[#1b0f0c] p-5 text-left text-[#ffeede] shadow-lg shadow-black/30 backface-hidden rotate-y-180",
        ...
        >
          {special.image_url && (
            <>
              ...
              <span
                className="absolute inset-0 bg-linear-to-b from-[#1b0f0c]/65 via-[#1b0f0c]/90 to-[#1b0f0c]"
                aria-hidden="true"
              />
            </>
          )}
          ...
          <div className="relative z-10 flex flex-1 flex-col">
            ...
            <button
              ...
              className="group/back mt-auto ml-auto inline-flex min-h-11 items-center justify-end gap-2 pt-3 font-black text-[10px] tracking-widest text-[#FDCC4B] uppercase focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#FDCC4B]"
            >
              Back
              <FlipHorizontal2
                className="h-5 w-5 transition-transform duration-300 group-hover/back:rotate-y-180"
                aria-hidden="true"
              />
            </button>
          </div>
