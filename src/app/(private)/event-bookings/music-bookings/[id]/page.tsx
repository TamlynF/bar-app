...
function DetailRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex items-start justify-between gap-4 border-b border-[#D8D5C8] py-3 last:border-0">
      <span className="shrink-0 pt-0.5 font-black text-[10px] tracking-wide text-[#5E6654] uppercase">
        {label}
      </span>
      <span className="text-right text-sm font-bold text-[#20231A]">{value || "-"}</span>
    </div>
  );
}
      ...
      <div className="mx-auto max-w-4xl space-y-6 px-4 py-4 sm:py-0 md:px-8">
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h1 className="font-black text-2xl leading-tight tracking-tight text-[#20231A] uppercase">
                {request.group_name || request.booker_name}
              </h1>
              {request.group_name && (
                <p className="mt-0.5 text-sm text-[#5E6654]">{request.booker_name}</p>
              )}
            </div>
            <span
              className={cn(
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            <div className="overflow-hidden rounded-2xl border border-[#D8D5C8] bg-white px-4 shadow-sm">
              ...
              {dates.length > 0 ? (
                <div className="flex items-start justify-between gap-4 py-3">
                  ...
                  <ul className="space-y-1 text-right">
                    {dates.map((d, i) => (
                      <li key={i} className="text-sm font-bold text-[#20231A]">
            ...
            <div className="overflow-hidden rounded-2xl border border-[#D8D5C8] bg-white px-4 shadow-sm">
              ...
              <div className="flex items-start justify-between gap-4 border-b border-[#D8D5C8] py-3">
                ...
                <a
                  ...
                  className="text-right text-sm font-bold break-all text-[#34451F] underline underline-offset-2"
                >
                  {request.email}
                </a>
              </div>
              {request.phone_no && (
                <div className="flex items-start justify-between gap-4 border-b border-[#D8D5C8] py-3">
                  ...
                  <a
                    ...
                    className="text-sm font-bold text-[#34451F] underline underline-offset-2"
                  >
                    {request.phone_no}
                  </a>
                </div>
              ...
              {socials.map(([key, url]) => {
                ...
                return (
                  <a
                    ...
                    className="flex items-center gap-1.5 rounded-xl border border-[#D8D5C8] bg-white px-3 py-2 text-xs font-bold text-[#34451F] shadow-sm transition-colors hover:bg-[#F4F1E8]"
                  >
        ...
        {request.notes && (
          <div className="space-y-2">
            ...
            <p className="rounded-2xl border border-[#D8D5C8] bg-white px-4 py-3 text-sm text-[#20231A] shadow-sm">
              {request.notes}
            </p>
          </div>
        )}
