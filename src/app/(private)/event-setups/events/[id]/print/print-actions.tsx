...
const openPrintDialog = () => requestAnimationFrame(() => setTimeout(() => window.print(), 0));

export default function PrintActions({ eventId }: { eventId: number }) {
  const autoPrint = useSearchParams().get("auto") === "1";

  useEffect(() => {
    if (autoPrint) openPrintDialog();
  }, [autoPrint]);

  return (
    <div data-print-hide className="mb-5 hidden items-center justify-between gap-3 sm:flex">
      <Link
