        ...
        <div className="mx-auto w-full max-w-3xl space-y-3 p-4 sm:p-6">
            {requestItems.map((item) => (
                <Link
                    key={item.href}
                    href={item.href}
                    className="group relative flex items-center gap-4 rounded-2xl border border-[#D8D5C8] bg-white p-4 shadow-sm transition-all hover:border-[#34451F] hover:shadow-md active:scale-[0.98]"
                >
                    <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${item.color}`}>
                        <item.icon className="h-5 w-5" />
                    </div>
                    <div className="min-w-0 flex-1">
                        <h3 className="font-black text-sm tracking-tight text-[#20231A] uppercase">{item.title}</h3>
                        <p className="truncate text-xs font-medium text-[#5E6654]">{item.description}</p>
                    </div>
                    <ChevronRight className="h-4 w-4 shrink-0 text-[#5E6654] group-hover:text-[#34451F]" />

                    {item.count > 0 && (
                        <span
                            aria-label={`${item.count} awaiting action`}
                            className="absolute -top-2 -right-2 flex h-6 min-w-6 items-center justify-center rounded-full bg-red-600 px-1.5 font-black text-[10px] text-white uppercase shadow-sm ring-2 ring-[#F4F1E8] tabular-nums"
                        >
