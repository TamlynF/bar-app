...
const CONFIG_FIELDS: {
  ...
  { key: "tickIntervalSec", label: "Tick interval (seconds)", step: "5", hint: "How often prices move" },
  { key: "noiseSigma", label: "Volatility", step: "0.005", hint: "Random wobble per tick" },
  { key: "floorPct", label: "Price floor (× base)", step: "0.05", hint: "0.7 = never below 70%" },
  { key: "ceilPct", label: "Price ceiling (× base)", step: "0.05", hint: "1.5 = never above 150%" },
  { key: "moveNotifyPct", label: "Alert threshold", step: "0.01", hint: "0.05 = alert on a 5% move" },
  { key: "lowStockThreshold", label: "Low stock at", step: "1", hint: "Units left before 'running low'" },
];
    ...
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {CONFIG_FIELDS.map((field) => (
        <label key={field.key} className="block">
          <span className="mb-1 block text-[11px] font-semibold text-admin-muted">
            {field.label}
          </span>
          <input
            type="number"
