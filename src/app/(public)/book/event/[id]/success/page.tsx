...
async function confirmAndNotify(bookingId: string) {
  ...
  try {
    ...
    if (!tender) return { status: "pending" as const, booking };
    ...
    if (contact?.email) {
      ...
      await resend.emails.send({
        ...
        to: contact.email,
        subject: `Booking Confirmed: ${event?.title ?? "Event"} 🎫`,
        html: `
          <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; color: #1f2937;">
            <div style="background-color: #ffffff; padding: 40px; border-radius: 8px; border: 1px solid #e5e7eb;">
              ...
              <p>Your booking is confirmed and paid. See you there!</p>
...
export default async function EventSuccessPage({
  ...
  if (!bookingId) {
    return <ErrorPage eventId={id} message="No booking reference found." />;
  }

  const result = await confirmAndNotify(bookingId);

  if (result.status === "not_found") {
    return <ErrorPage eventId={id} message="Booking not found. Please contact us." />;
  }
  ...
  if (result.status === "waitlisted") {
    return (
      <StatusPage
        ...
        message="Your payment went through, but the last table for this event was taken while you were checking out. We'll contact you to sort out a table or a refund."
        eventTitle={eventTitle}
        eventDate={eventDate}
        groupSize={booking.group_size}
        total={booking.total_amount}
